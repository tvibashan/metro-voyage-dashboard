...
import PricingTiersModal from "./PricingModal";
import { toast } from "sonner";

export default function OptionsModal({ onNext }: { onNext: () => void }) {
  const [step, setStep] = useState(1);
  const { optionData, setOptionData }: any = useOptionStore();
  ...
  const handleNextStep = async () => {
    ...
    if (step === 1) {
      if (!scheduleName.trim()) {
        toast("Please provide a schedule name.");
        return;
      }
      if (!startDate) {
    ...
    if (step === 2) {
      if (availabilityType === "fixed") {
        const hasFixedSlots = fixed_time_slots.some(
          (day) => day.slots.length > 0
        );
        if (!hasFixedSlots) {
          toast("Please add at least one fixed time slot.");
          return;
        }
      } else if (availabilityType === "operating") {
        const hasOperatingHours = operating_hours.some(
          (day) => day.hours.length > 0
        );
        if (!hasOperatingHours) {
          toast("Please add at least one operating hour.");
          return;
        }
      }
    }
    ...
    // Step 3 Validation
    if (step === 3) {
      if (currentAvailability.pricing_tiers?.length === 0) {
        toast("Please define at least one pricing tier.");
        return;
      }
    }
  ...
  return (
    <div className="w-full h-full">
      ...
      <div className="flex gap-2 mb-4">
        {optionData?.availabilities.map((avb: any, index: number) => (
          <div key={index} className="flex items-center gap-2">
            ...
            >
              Availability {avb.schedule_name}
            </Button>
            <Button
      ...
      {step === 1 && (
        <div className="space-y-6">
          <div>
            <Label>
              Schedule Name <span className="text-sm text-red-500">*</span>
            </Label>
            <Input
              value={scheduleName}
