...
export default function OptionsModal({ onNext }: { onNext: () => void }) {
  ...
  const isAttractionTicket = productData.category === "Attraction ticket";
  const [selectedLanguage, setSelectedLanguage] = useState("");
  const [selectedBookletLanguage, setSelectedBookletLanguage] = useState("");
  const [selectedAudioGuideLanguage, setSelectedAudioGuideLanguage] =
    useState("");
  ...
  const handleLanguageSelect = (value: string) => {
    ...
    if (
      value &&
      !optionData.host_languages?.some((lang) => lang.keyword === value)
    ) {
  ...
  const handleBookletLanguageSelect = (value: string) => {
    ...
    if (
      value &&
      !optionData.booklet_languages?.some((lang) => lang.keyword === value)
    ) {
      const updatedLanguages = [
  ...
  const handleAudioGuideLanguageSelect = (value: string) => {
    ...
    if (
      value &&
      !optionData.audio_guides_languages?.some((lang) => lang.keyword === value)
    ) {
      const updatedLanguages = [
  ...
  const handleNextStep = async () => {
    if (step === 1) {
      if (!optionData.name || !optionData.maximum_group_size) {
        toast("Please fill out all required fields in Step 1.");
        return;
      }
    } else if (step === 2) {
      if (
        optionData.skip_the_line_enabled &&
        !optionData.skip_the_line_option
      ) {
        toast("Please select a ticket option for 'Skip the Line'.");
        return;
      }
    }
        ...
        <div className="space-y-4">
          ...
          <div>
            <label
              ...
              className="block mb-2 text-sm font-medium"
            >
              Maximum group size <span className="text-sm text-red-500">*</span>
            </label>
            <input
              id="groupSize"
              type="number"
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {optionData.booklet_languages?.map((lang) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    handleRemoveLanguage(lang.keyword, "booklet")
                  }
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {optionData.audio_guides_languages?.map((lang) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    handleRemoveLanguage(lang.keyword, "audio_guide")
                  }
                  className="focus:outline-none"
                >
                  <Icon
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {optionData.host_languages?.map((lang) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    handleRemoveLanguage(lang.keyword, "host")
                  }
                  className="focus:outline-none"
                >
                  <Icon
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <div className="mb-4">
            ...
            {/* Show Select Ticket Option if "Yes" is selected */}
            {optionData.skip_the_line_enabled && (
              <div className="mt-4">
                <label
                  className="block mb-2 text-sm font-medium"
