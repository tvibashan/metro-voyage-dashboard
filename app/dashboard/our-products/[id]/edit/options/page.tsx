...
import MeetingPoint from "./modals/MeetingPointModal";
import DurationValidity from "./modals/DurationValidityModal";
...
const Option: React.FC = () => {
  const { productData } = useProductEditStore();
  ...
  useEffect(() => {
    if (productData?.option) {
      setOptionData(productData?.option);
    }
  }, []);
    ...
    <div className="pt-2 h-screen">
          ...
          {optionData && (
            <div className="">
              ...
              <div className="p-4 border rounded-lg bg-blue-100 border-blue-500 flex justify-between items-center">
                <h5 className="font-medium">{optionData.name}</h5>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="w-screen h-screen max-w-full max-h-full border-none outline-none flex flex-col md:flex-row overflow-hidden rounded-none p-0">
          ...
          <div className="w-full h-screen overflow-y-auto p-4 md:p-6">
            ...
            {activeModal === "Review" && (
              <ReviewModal setIsModalOpen={setIsModalOpen} />
            )}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isSummaryModalOpen} onOpenChange={setIsSummaryModalOpen}>
        <DialogContent className="sm:max-w-[800px] h-[600px] flex p-0 rounded-3xl">
          <OptionSummaryModal data={optionData} />
        </DialogContent>
      </Dialog>
