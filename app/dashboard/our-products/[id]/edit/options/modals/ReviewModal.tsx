...
export default function ReviewModal({setIsModalOpen}: { setIsModalOpen: (isOpen: boolean) => void }) {
  const { submitOption, optionData }:{submitOption:any,optionData:OptionData} = useEditOptionStore();
  ...
  const handleSubmit = async () => {
    ...
    await submitOption(optionData?.id, () => setIsModalOpen(false));
  };
      ...
      <Card className="mb-8 hover:shadow-lg transition-shadow duration-200">
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <RowColumn
              label="Skip the Line"
              value={optionData.skip_the_line ? "Yes" : "No"}
              status={optionData.skip_the_line ? "success" : "error"}
            />
            <RowColumn
              label="Audio Guide"
