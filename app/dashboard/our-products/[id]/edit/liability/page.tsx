...
const Liability = ({ fetchProductDetails }: any) => {
  const { prevStep } = useEditStepStore();
  const { productData, setProductData, submitProduct } = useProductEditStore();
  const [isProductActive, setIsProductActive] = useState(
    productData.status || false
  );
  // const [cancellationPolicy, setCancellationPolicy] = useState(
  //   productData.cancellationPolicy || ""
  // );
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      await submitProduct(productData.id, fetchProductDetails);
    } catch (error) {
