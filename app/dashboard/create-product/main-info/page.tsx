...
const MainInfo: React.FC = () => {
  ...
  // Converts "HH:MM" format to "X hours Y minutes" display format
  const formatDurationForDisplay = (duration: any) => {
    if (!duration) return "";
    ...
    const hoursText =
      hoursInt > 0 ? `${hoursInt} ${hoursInt === 1 ? "hour" : "hours"}` : "";
    const minutesText =
      minutesInt > 0
        ? `${minutesInt} ${minutesInt === 1 ? "minute" : "minutes"}`
        : "";

    return [hoursText, minutesText].filter(Boolean).join(" ");
  };

  // Converts "X hours Y minutes" back to "HH:MM" format for state
  const formatDurationForState = (displayDuration: any) => {
  ...
  return (
    <div className="pb-4">
      <div className="w-full mx-auto">
        ...
        <div className="mt-6 bg-white border rounded-[20px]">
          <div className="flex items-center justify-between p-6 border-b w-full py-4">
            <h2 className="text-xl font-semibold">
              Uploaded Product Photo <span className="text-red-500">*</span>
              <span className="block text-sm font-normal text-[#010A15B2]">
        ...
        {/* Duration - Simple input version with separate hour/minute fields */}
        <div className="mt-6 bg-white border rounded-[20px]">
          <div className="flex items-center justify-between p-6 border-b w-full py-4">
            ...
            <div className="flex items-center gap-4 mt-4">
              <div className="flex-1">
                ...
                <input
                  ...
                  value={
                    duration
                      ? formatDurationForState(duration).split(":")[0]
                      : ""
                  }
                  onChange={(e) => {
                    ...
                    const current = formatDurationForState(duration);
                    const minutes = current ? current.split(":")[1] : "00";
                    setDuration(
                      formatDurationForDisplay(`${hours}:${minutes}`)
                    );
                  }}
                  min="0"
                  max="99"
                  placeholder="0"
                  className="w-full p-4 border rounded-[12px] focus:outline-none focus:border focus:border-green-500 focus:border-opacity-30"
                />
              </div>
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  value={
                    duration
                      ? formatDurationForState(duration).split(":")[1]
                      : ""
                  }
                  onChange={(e) => {
                    ...
                    const current = formatDurationForState(duration);
                    const hours = current ? current.split(":")[0] : "0";
                    setDuration(
